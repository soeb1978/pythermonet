...
class THERMONET:
    ...
    def __init__(self, PWD,dpt,lsh,lsc,rhocs,zd):
        self.PWD = PWD
        self.dpt = dpt
        self.lsh = lsh
        self.lsc = lsc
...
def ThermonetDim(PID,HPFN,TOPOFN,BRINE,LPE,THERMONET,HEAT_PUMPS,SOURCE_SELECTION,SOURCE):
    ...
    TOPOFN = TOPOFN;                                          # Input file containing topology information 
    
    # Brine
    rhob = BRINE.rho;                                                         # Brine density (kg/m3), T = 0C. https://www.handymath.com/cgi-bin/isopropanolwghtvoltble5.cgi?submit=Entry
    cb = BRINE.c;                                                          # Brine specific heat (J/kg/K). 4450 J/kg/K is loosly based on Ignatowicz, M., Mazzotti, W., Acuña, J., Melinder, A., & Palm, B. (2017). Different ethyl alcohol secondary fluids used for GSHP in Europe. Presented at the 12th IEA Heat Pump Conference, Rotterdam, 2017. Retrieved from http://urn.kb.se/resolve?urn=urn:nbn:se:kth:diva-215752
    mub = BRINE.mu;                                                         # Brine dynamic viscosity (Pa*s). Source see above reference.
    lb = BRINE.l;                                                          # Brine thermal conductivity (W/m/K). https://www.researchgate.net/publication/291350729_Investigation_of_ethanol_based_secondary_fluids_with_denaturing_agents_and_other_additives_used_for_borehole_heat_exchangers
    
    # PE Pipes
    lp = LPE;                                                           # Pipe thermal conductivity (W/m/K). https://www.wavin.com/da-dk/catalog/Varme/Jordvarme/PE-80-lige-ror/40mm-jordvarme-PE-80PN6-100m
    
    # Thermonet and HHE
    PWD = THERMONET.PWD;                                                          # Distance between forward and return pipe centers (m)
    dpt = THERMONET.dpt;                                                           # Target pressure loss in thermonet (Pa/m). 10# reduction to account for loss in fittings. Source: Oklahoma State University, Closed-loop/ground source heat pump systems. Installation guide., (1988). Interval: 98-298 Pa/m
    ...
    Tci = HEAT_PUMPS.Tci;                                                           # Design temperature for inlet (C) OK. Stress test conditions. Legislation stipulates Thi > -4C. Auxillary heater must be considered.
    SF = HEAT_PUMPS.SF;                                                             # Ratio of peak heating demand to be covered by the heat pump [0-1]. If SF = 0.8 then the heat pump delivers 80% of the peak heating load. The deficit is then supplied by an auxilliary heating device
    
    # Source selection
    SS = SOURCE_SELECTION;                                                             # SS = 1: Borehole heat exchangers; SS = 0: Horizontal heat exchangers  
