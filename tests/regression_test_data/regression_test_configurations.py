from thermonet.dimensioning.dimensioning import run_dimensioning, DimensioningConfgiuration, HorizontalConfiguration, BTESConfiguation
...
basic_configuration = DimensioningConfgiuration(
    ...
    # Input files
    HPFN='../src/thermonet/dimensioning/examples/data/sites/Silkeborg_HPSC.dat',  # Input file containing heat pump information
    TOPOFN='../src/thermonet/dimensioning/examples/data/sites/Silkeborg_TOPO.dat',  # Input file containing topology information
    ...
    lb=0.45,
    # Brine thermal conductivity (W/m/K). https://www.researchgate.net/publication/291350729_Investigation_of_ethanol_based_secondary_fluids_with_denaturing_agents_and_other_additives_used_for_borehole_heat_exchangers

    # PE Pipes
    lp=0.4,
    # Pipe thermal conductivity (W/m/K). https://www.wavin.com/da-dk/catalog/Varme/Jordvarme/PE-80-lige-ror/40mm-jordvarme-PE-80PN6-100m

    # Thermonet and HHE
    PWD=0.3,  # Distance between forward and return pipe centers (m)
    dpt=90,
    # Target pressure loss in thermonet (Pa/m). 10# reduction to account for loss in fittings. Source: Oklahoma State University, Closed-loop/ground source heat pump systems. Installation guide., (1988). Interval: 98-298 Pa/m
...
with open("regression_test_data/result_basic_configuration.txt") as f:
    basic_output = f.read()

test_configurations = {
    "basic": (
        basic_configuration, basic_output
